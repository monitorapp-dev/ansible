...
---
module: opmode_set
short_description: Get operation mode.
description:
    - This module can check the operating mode of AIWAF.
...
options:
    ...
    login_pwd:
        ...
        required: true
    mode:
        description: When the operation mode is "Session and policy bypass",
                     AIWAF does not establish sessions but bypass TCP packets. (AIWAF does not involve in TCP sessions.)
                     When the operation mode is "policy bypass", AIWAF bypasses incoming data after establish sessions.
                     When the operation mode is "Detection mode", AIWAF operates in Detection mode even though the set action for policies is set to "Block".
        type: str
        choices: ['sess_bypass', 'insp_bypass', 'active', 'passive']
        required: true
'''
...
def main():
    module_args = dict(
        ...
        mode=dict(type='str', required=True, choices=['sess_bypass', 'insp_bypass', 'active', 'passive']),
    )
    ...
    url = 'https://localhost:223/v1/policy/base/opmode'
